...
import config_init
import db_operations  # Import db_operations to save data into the database
...
def search_flights():
    ...
    if response.status_code == 200:
        try:
            ...
            print(etree.tostring(root, pretty_print=True).decode())  # Print full XML response for inspection

            flight_info = []
            namespaces = {"shared": "http://ypsilon.net/shared"}

            # Loop through the 'fare' elements
            for fare in root.xpath("//fare", namespaces=namespaces):
                ...
                # Save flight information in the database
                flight_data = {
                    "fare_id": fare_id,
                ...
                if db_operations.save_flight(flight_data):
                    # After saving the flight, save passengers
                    for pax in passengers:
                        db_operations.save_passenger(pax, fare_id)

                # Append flight info to the response
                flight_info.append({
                    "fare_id": fare_id,
                    "departure_airport": dep_apt,
                    "destination_airport": dst_apt,
